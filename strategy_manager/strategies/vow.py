...
def manage_strategy(client_id, strategy_manager):
    try:
        ...
        strategy.run()# loop.run_until_complete(strategy.run()) # strategy.run()
...
class Strategy:
    ...
    def on_fill(self, trade, fill):
        ...
        self.strategy_manager.message_queue.put({
            ...
            'trade': trade,
            'fill': fill,
            'info': f'f"{trade.fills[0].execution.side} {trade.orderStatus.filled} {trade.contract.symbol}@{trade.orderStatus.avgFillPrice} [{self.strategy_symbol}]" '
        })
    ...
    def run(self):
        ...
        trade = self.trade_manager.trade(contract,1,order_type='MKT',orderRef=self.strategy_symbol,algo = True,urgency="Normal",useRth=True)
